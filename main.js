async function getPrecipitations(latitude, longitude, timezone, forecastDays, pastDays) {
  ...
  function update() {
    ...
    const [selectedLatitude, selectedLongitude] = document.getElementById("city").value.split(",").map(Number);
    ...
    console.log(`Fetching data for pastDays: ${pastDays}, forecastDays: ${forecastDays} at latitude: ${selectedLatitude}, longitude: ${selectedLongitude}`);

    getPrecipitations(selectedLatitude, selectedLongitude, timezone, forecastDays, pastDays).then(data => {
